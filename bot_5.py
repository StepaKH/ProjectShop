...
def callback_message(callback):
    if callback.data == 'status':
        ...
        while check_file_lock('cards.xlsx'):
            ...
            time.sleep(1)
        wb = openpyxl.load_workbook('cards.xlsx', read_only=True, keep_vba=True)
        try:
            ws = wb.active
            for row in ws.iter_rows(min_row=2, max_col=10, values_only=True):
                if str(row[9]) == user_states[callback.message.chat.id]['phone']:
            ...
            if float(user_states[message.chat.id]['size']) <= float(
                ...
                bot.send_photo(config.manager_id, img,
                               caption=f'Принять заказ!\n'
                                       ...
                                       f'Название: {user_states[message.chat.id]['product_data'][1]}\n'
                                       f'Цена: {user_states[message.chat.id]['product_data'][2]}\n'
                                       f'Ширина: {user_states[message.chat.id]['size']} {user_states[message.chat.id]['product_data'][3].split(" ")[1]}\n\n'
